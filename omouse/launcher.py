...
ENV_OPTIONS = "OMARCHY_MOUSE_OPTIONS"
SHARE_DIRS = ("/usr/share/omarchy-mouse-utils/qml", "/usr/local/share/omarchy-mouse-utils/qml")
...
def run(name: str, options: dict, wait: bool = True) -> int:
    ...
    binary = quickshell()
    if binary is None:
        print("omarchy-mouse-utils: quickshell is not installed (sudo pacman -S quickshell)",
              file=sys.stderr)
        return 127
    path = qml_file(name)
...
def already_running(namespace: str = "omarchy-mouse-utils") -> bool:
